...
def main():
    ...
    LABELS = ['Violin_or_fiddle', 'Saxophone']
    ...
    # lst_learning_rates = [lr/10,lr*10, lr/100, lr*100]
    lst_audio_length = [audio_duration+1, audio_duration-1, audio_duration+2]

    for ad in lst_audio_length:
        for i in range(len(lst_of_networks)):
            print('--------------Starting training ', network_names[i]+'audio_dur'+str(ad))
            try:
                train_network(network_names[i]+'audio_dur_'+str(ad),lst_of_networks[i],config, train, test, LABELS, num_cores)
                print('################################################################################\n'*20)
            except:
                print('#$'*20)
                print('ERROR with '+str(network_names[i])+'audio_dur_'+str(ad))
